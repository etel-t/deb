...
def main():
    ...
    '''.format(pa, version))
                ...
                # build deb package
                system2(
                    'cp target/release/bundle/deb/reachdesk*.deb ./reachdesk.deb')
                system2('dpkg-deb -R reachdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/reachdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/reachdesk.svg')
                system2(
                    'cp res/reachdesk.desktop tmpdeb/usr/share/applications/reachdesk.desktop')
                system2(
                    'cp res/reachdesk-link.desktop tmpdeb/usr/share/applications/reachdesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/reachdesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/reachdesk/')
                os.system('mkdir -p tmpdeb/etc/X11/reachdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/reachdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/reachdesk.debian tmpdeb/etc/pam.d/reachdesk')
                ...
                system2('cp libsciter-gtk.so tmpdeb/usr/lib/reachdesk/')
                md5_file('usr/share/reachdesk/files/systemd/reachdesk.service')
                md5_file('etc/reachdesk/startwm.sh')
                md5_file('etc/X11/reachdesk/xorg.conf')
                md5_file('etc/pam.d/reachdesk')
                md5_file('usr/lib/reachdesk/libsciter-gtk.so')
                system2('dpkg-deb -b tmpdeb reachdesk.deb')
                os.rename('reachdesk.deb', 'reachdesk-%s.deb' % version)
